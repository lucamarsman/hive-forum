...
class Comment {
  ...
  static async updateComment(req, res) {
    ...
    try {
      await queryDb("UPDATE comments SET content = ? WHERE comment_id = ?", [
        newContent,
        commentId,
      ]);
      res
  ...
  static async fetchPostComments(req, res) {
    ...
    const comments = await queryDb(
      "SELECT * FROM comments WHERE post_id = ? AND parent_id IS NULL ORDER BY comment_id DESC LIMIT ? OFFSET ?",
      [postId, limit, offset],
    );
...
async function fetchReplies(parentId, uid) {
  // fetch replies for a comment helper function
  const replies = await queryDb("SELECT * FROM comments WHERE parent_id = ?", [
    parentId,
