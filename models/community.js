...
class Community {
    ...
    if (res.authenticated) {
      ...
      if (req.file) {
        result = await queryDb(
          "INSERT INTO communities (name, description, created_at, created_by, logo_path) VALUES (?,?,?,?,?)",
          [
            payload.title,
            payload.description,
            payload.createdAt,
      ...
      await queryDb(
        "INSERT INTO communitymemberships (user_id, community_id, role, joined_at) VALUES (?,?,?,?)",
        [uid, insertedId, "moderator", payload.createdAt],
      );
  ...
  static async fetchCommunityDetails(req, res) {
    ...
    const details = await queryDb("SELECT * FROM communities WHERE name = ?", [
      communityName,
    ]);
    res.json(details);
  }
  ...
  static async fetchCommunityDetailsViaId(req, res) {
    ...
    const details = await queryDb("SELECT * FROM communities WHERE id = ?", [
      communityId,
    ]);
    res.json(details);
  }
  ...
  static async fetchCommunities(req, res) {
    ...
    try {
      const communities = await queryDb(
        `SELECT name, logo_path, description FROM communities LIMIT ? OFFSET ?`,
        [limit, offset],
      );
  ...
  static async searchCommunities(req, res) {
    ...
    try {
      const communities = await queryDb(
        `SELECT name, logo_path, description FROM communities WHERE name LIKE ? LIMIT ? OFFSET ?`,
  ...
  static async joinCommunity(req, res) {
    if (res.authenticated) {
      try {
        console.log("joinCommunity URL:", req.originalUrl);
        console.log("joinCommunity query:", req.query);
        ...
        if (!communityResult || communityResult.length === 0) {
          console.log("joinCommunity: community not found", {
            rawUrl: req.originalUrl,
            communityName,
          });
          return res
            .status(404)
            .json({ error: "Community not found", communityName });
        }
        ...
        await queryDb(
          "INSERT INTO communitymemberships (user_id, community_id, role, joined_at) VALUES (?,?,?,?)",
          [uid, communityId, "member", new Date(new Date().getTime())],
        );
  ...
  static async leaveCommunity(req, res) {
    if (res.authenticated) {
      try {
        ...
        await queryDb(
          "DELETE FROM communitymemberships WHERE user_id = ? AND community_id = ?",
          [uid, communityId],
        );
        res.sendStatus(200);
      } catch (error) {
  ...
  static async checkMembership(req, res) {
    if (res.authenticated) {
      try {
        ...
        const isMember = await queryDb(
          "SELECT * FROM communitymemberships WHERE user_id = ? AND community_id = ?",
          [uid, communityId],
        );
  ...
  static async getCommunityStats(req, res) {
    ...
    const memberCountResult = await queryDb(
      "SELECT COUNT(*) AS total_members FROM communitymemberships WHERE community_id = ?",
      [communityId],
    );
  ...
  static async getPopular(req, res) {
    try {
      ...
      res.status(500).json({
        error: "An error occurred while fetching popular communities.",
      });
    }
  }
  ...
  static async getLogo(req, res) {
    try {
      const results = await queryDb(
        `SELECT logo_path FROM communities WHERE name = ?`,
