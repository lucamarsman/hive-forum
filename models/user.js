...
const {
  ...
  generateResetToken,
  validateUser,
} = require("../Auth.js"); // Import Auth.js functions
...
async function savePost(postId, userId) {
  ...
  try {
    ...
    if (rows.length > 0) {
      // If the post has already been saved
      ...
      // If the post has not already been saved
      await queryDb("INSERT INTO saves (user_id, post_id) VALUES (?,?)", [
        userId,
        postId,
      ]); // Save the post
...
async function likePost(postId, userId) {
  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      // If the post has not already been liked
      await queryDb("INSERT INTO likes (user_id, post_id) VALUES (?,?)", [
        userId,
        postId,
      ]); // Like the post
...
class User {
  ...
  static async create(username, password, email) {
    ...
    const query =
      "INSERT INTO users (username, password, email) VALUES (?, ?, ?)"; // Insert the user into the database
  ...
  static async register(req, res) {
    ...
    if (!accountExists) {
      ...
      res.cookie("register-token", register_link, {
        // Set register token cookie
      ...
      await queryDb(
        "INSERT INTO registry (email, password, username, register_link, register_link_timestamp) VALUES (?,?,?,?,?)",
        [email, hashedPassword, username, register_link, regLinkTs],
      );
      ...
      return res
        .status(200)
        .send("A confirmation email has been sent to the email submitted"); // Send 200 back to client
    } else {
  ...
  static async registerConfirm(req, res) {
    ...
    if (!req.cookies["register-token"]) {
      return res
        .status(400)
        .send("Confirmation window expired, please try again");
    }

    // Retrieve user information based on the token in the registry table
    const pendingUser = await queryDb(
      "SELECT email, password, username, register_link FROM registry WHERE register_link = ?",
      [token],
    );
  ...
  static async getUserImage(req, res) {
    ...
    const imageURL = await queryDb(
      "SELECT image_path FROM profilepictures WHERE user_id = ?",
      [userId],
    ); // Get user profile image from database
    res.json(imageURL); // Send user profile image to client
  }

  static async getPosterImage(req, res) {
    // Get user profile image
    try {
  ...
  static async saveProfile(req, res) {
    // Save user profile bio
    let decodedToken = jwt_decode(req.cookies["refresh-token"]); // Decode JWT
    const uid = decodedToken.user.userid; // Get user id from decoded token

    await queryDb("UPDATE users SET bio = ? WHERE user_id = ?", [
      req.body.bio,
      uid,
  ...
  static async uploadImage(req, res) {
    ...
    try {
      ...
      if (req.file) {
        ...
        let decodedToken = jwt_decode(req.cookies["refresh-token"]); // Decode JWT
        const userId = decodedToken.user.userid; // Get user id from decoded token
        const picQuery = await queryDb(
          "SELECT * FROM profilepictures WHERE user_id = ?",
          [userId],
        ); // Check if the user already has a profile picture
        if (picQuery.length === 0) {
          // If the user does not have a profile picture
          await queryDb(
            "INSERT INTO profilepictures (image_path, user_id) VALUES (?, ?)",
            [req.file.path, userId],
          ); // Insert the profile picture into the database
        } else {
          await queryDb(
            "UPDATE profilepictures SET image_path = ? WHERE user_id = ?",
            [req.file.path, userId],
      ...
      res
        .status(500)
        .json({ success: false, message: "Internal server error." }); // Send 500 back to client
    }
  }
  ...
  static async resetPassword(req, res) {
    ...
    if (newPassword === confirmNewPassword) {
      ...
      await queryDb("UPDATE users SET password = ? WHERE reset_link = ?", [
        newHash,
        reToken_payload,
      ]);
      await queryDb("UPDATE users SET reset_link = NULL WHERE reset_link = ?", [
        reToken_payload,
      ]);
  ...
  static async getResetLink(req, res) {
    ...
    const rows = await queryDb("SELECT email FROM users WHERE email = ?", [
      req.body.email,
    ]); // Check if the email is registered
    if (rows.length > 0) {
      // If the email is registered
      await queryDb("UPDATE users SET reset_link = ? WHERE email = ?", [
        reset_link,
        req.body.email,
      ]); // Set the reset link in the database
      const userId = (
        await queryDb("SELECT user_id FROM users WHERE email = ?", [
          req.body.email,
        ])
      )[0].user_id; // Get the user id from the database
  ...
  static async login(req, res) {
    ...
    if (rows.length > 0) {
      ...
      const passwordResult = await bcrypt.compare(
        req.body.password,
        result[0].password,
      ); // Compare the password from the database with the password from the request body
      const userIdResult = await queryDb(
    ...
    if (res.authenticated) {
      ...
      try {
        ...
        if (likeResult.alreadyLiked) {
          ...
          res
            .status(300)
            .send({ success: true, message: "Post already liked" }); // Send 300 back to client
          await queryDb("DELETE FROM likes WHERE user_id = ? AND post_id = ?", [
            userId,
            postId,
          ]); // Delete the post from the likes table
          await queryDb(
            "UPDATE posts SET likeCount = likeCount - 1 WHERE post_id = ?",
            [postId],
          ...
          res
            .status(200)
            .send({ success: true, message: "Post liked successfully" });
          await queryDb(
            "UPDATE posts SET likeCount = likeCount + 1 WHERE post_id = ?",
            [postId],
          );
    ...
    if (res.authenticated) {
      ...
      try {
        ...
        if (saveResult.alreadySaved) {
          ...
          await queryDb("DELETE FROM saves WHERE user_id = ? AND post_id = ?", [
            userId,
            postId,
          ]); // Delete the post from the saves table
        } else {
          // If the post has not already been saved
          res
            .status(200)
            .send({ success: true, message: "Post saved successfully" }); // Send 200 back to client
        }
      } catch (error) {
        // Catch any errors
  ...
  static async fetchSaveHistory(req, res) {
    // Fetch user's post save history
    try {
  ...
  static async fetchLikeHistory(req, res) {
    // Fetch user's post like history
    try {
      const limit = 5; // number of posts per page
      const page = req.query.page ? parseInt(req.query.page) : 1; // Get page number from request query
      const offset = (page - 1) * limit; // Calculate offset
